...
import { Divide, ShoppingBagIcon, Trash2 } from "lucide-react";
  ...
  return (
    <div>
       ...
       <div className="flex gap-2 font-montserrat items-center py-5">
        ...
        <h1 className="text-2xl text-hashblack font-montserrat font-semibold">Shopping Cart</h1>
       </div>
        <div className="grid  lg:grid-cols-3 md:gap-8 ">
          <div className="lg:col-span-1 font-montserrat">
            <div className="hidden md:inline-block w-full rounded-lg border p-6  bg-slate-50">
              <h2 className="text-xl mb-4 text-hashblack font-semibold" >Order Summary</h2>
              <div className="space-y-4">
               ...
               <div className="flex items-center  justify-between space-x-3 ">
                ...
                <Button onClick={handleCheckoutProccess}>Proceed to Checkout</Button>
                <Link href={'/'} className="font-medium text-center text-primary text-sm hover:underline hoverEffect ">Coutinue Shoppping</Link>
                </div>
              </div>
            </div>
          </div>
          <div className="lg:col-span-2 font-montserrat">
            ...
            <div className=" grid grid-cols-5 md:grid-cols-6 border rounded-tr-lg rounded-tl-l bg-slate-50 p-2.5 text-sm sm:text-base font-semibold  text-hashblack ">
              ...
              {groupedItems.map(({product})=>{
                ...
                return (<div key={product?._id} className="grid grid-cols-5 md:grid-cols-6 border-b p-2.5  last:border-b-0">
                  ...
                  <div className="col-span-2 md:col-span-3 flex items-center">
                    ...
                    {product?.productImage && <Link href={`/product/${product?.slug?.current}`} className="border p-0.5 md:p-1 mr-2 rounded-md overflow-hidden group">
                      <Image
                      ...
                      /></Link>} 
                      <h2 className="text-xs sm:text-sm">{product?.product}</h2>
                  </div>
                  <div className=" text-sm md:text-lg flex justify-center items-center">
                    <PriceFormat amount={product?.price}/>
                  </div>
                  <ProductQuantityButton product={product} className="text-sm gap-0 md:gap-1 "/>
                  <div className=" text-sm md:text-lg flex justify-center items-center">
