...
const CartIcon = () => {
  ...
  return (
    ...
    >
      ...
      <span className="block md:hidden font-bold text-mahrron ">{getGroupedItems?.length ? getGroupedItems?.length : 0}</span>
      <div className="item-center hidden md:flex flex-col">
        <p className="text-xs">
          <span className="font-bold text-mahrron">{getGroupedItems?.length ? getGroupedItems?.length : 0}</span> items
        </p>
